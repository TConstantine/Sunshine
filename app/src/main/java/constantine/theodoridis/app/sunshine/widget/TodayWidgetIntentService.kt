...
class TodayWidgetIntentService : IntentService("TodayWidgetIntentService") {
	companion object {
		private val FORECAST_COLUMNS = arrayOf(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID, WeatherContract.WeatherEntry.COLUMN_SHORT_DESC, WeatherContract.WeatherEntry.COLUMN_MAX_TEMP, WeatherContract.WeatherEntry.COLUMN_MIN_TEMP)
	...
	override fun onHandleIntent(intent: Intent?) {
		...
		val data = contentResolver.query(weatherForLocationUri, FORECAST_COLUMNS, null, null, WeatherContract.WeatherEntry.COLUMN_DATE + " ASC")
						?: return
		if (!data.moveToFirst()) {
			data.close()
			return
